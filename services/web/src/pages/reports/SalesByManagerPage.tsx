...
import { useReportSalesByManager, useUsers } from "@/api/hooks";
...
export function SalesByManagerPage() {
  ...
  const { data, isLoading, isError, error } = useReportSalesByManager({
  ...
  const handleExportCSV = async () => {
    ...
    try {
      await downloadReportFile(
        "sales-by-manager" as ReportType,
        ...
        <>
          ...
          <div className="overflow-x-auto rounded-lg border">
            <table className="w-full min-w-[500px] text-sm">
              ...
              <tbody>
                {rows.map((r) => (
                  <tr key={r.managerId} className="border-b last:border-0">
                    <td className="px-4 py-2">{r.managerName}</td>
